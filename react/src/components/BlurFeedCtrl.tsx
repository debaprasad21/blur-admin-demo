// Converted from src/app/pages/dashboard/blurFeed/BlurFeedCtrl.js
...
const BlurFeedCtrl: React.FC = () => {
  ...
  const expandMessage = (index: number) => {
    setFeed(prevFeed => {
      ...
      newFeed[index].expanded = !newFeed[index].expanded;
      return newFeed;
    });
  };
    ...
    <div>
      {feed.map((message, index) => (
        <div key={index}>
          ...
          <p>{message.text}</p>
          <button onClick={() => expandMessage(index)}>
            {message.expanded ? 'Collapse' : 'Expand'}
          </button>
          {message.expanded && (
            <div>
              <p>Author: {message.author} {message.surname}</p>
              <p>Time: {message.time}</p>
              <p>Ago: {message.ago}</p>
              {message.preview && <img src={message.preview} alt="preview" />}
              {message.link && <a href={message.link}>Link</a>}
            </div>
          )}
        </div>
      ))}
