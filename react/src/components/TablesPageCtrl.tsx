// Converted from src/app/pages/tables/TablesPageCtrl.js

import React, { useState } from 'react';
import { Button } from 'react-bootstrap';
import { useFilter } from 'react-use';
...
const TablesPageCtrl: React.FC = () => {
  const [smartTablePageSize] = useState(10);
  const [smartTableData] = useState([
  ...
  const [editableTableData] = useState(smartTableData.slice(0, 36));

  const [peopleTableData] = useState([
  ...
  const [metricsTableData] = useState([
  ...
  const showGroup = (user: User) => {
    if (user.group && groups.length) {
      const selected = useFilter(groups, { id: user.group });
      return selected.length ? selected[0].text : 'Not set';
    } else return 'Not set';
  };

  const showStatus = (user: User) => {
    const selected = user.status ? useFilter(statuses, { value: user.status }) : [];
    return selected.length ? selected[0].text : 'Not set';
  };
  ...
  return (
    <div>
      {/* Render tables and other UI elements here */}
      <Button onClick={addUser}>Add User</Button>
    </div>
  );
};

export default TablesPageCtrl;
