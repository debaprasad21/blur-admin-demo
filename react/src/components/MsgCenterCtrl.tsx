// Converted from src/app/theme/components/msgCenter/MsgCenterCtrl.js
...
const MsgCenterCtrl: React.FC = () => {
  const [users] = useState<{ [key: number]: User }>({
    0: { name: 'Vlad' },
    1: { name: 'Kostya' },
    2: { name: 'Andrey' },
    3: { name: 'Nasta' },
  ...
  const getMessage = (msg: Notification) => {
    let text = msg.template;
    if (msg.userId !== undefined) {
      text = text.replace('&name', `<strong>${users[msg.userId].name}</strong>`);
    }
    ...
    <div>
      ...
      <ul>
        {notifications.map((notification, index) => (
          <li key={index} dangerouslySetInnerHTML={getMessage(notification)} />
        ))}
      </ul>
      <h2>Messages</h2>
      <ul>
        {messages.map((message, index) => (
          <li key={index}>
            <strong>{users[message.userId].name}:</strong> {message.text} <em>({message.time})</em>
          </li>
        ))}
      </ul>
    </div>
